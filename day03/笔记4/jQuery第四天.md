...
​		元素位置：offset，position

​		卷起距离：scrollTop，scrollLeft，scroll事件：谁有滚动条加给谁，(window特殊)
...
> triggerHandler模式不会触发事件的默认行为，这是和前面两种的区别。
...
​	事件注册：$(元素).click(function () {});

​	事件处理：$(元素).on('事件类型'，'后代元素'，function () {});
